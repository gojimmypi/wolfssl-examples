...
//    if ((ctx = wolfSSL_CTX_new(wolfSSLv23_client_method())) == NULL) {
    if ((ctx = wolfSSL_CTX_new(wolfTLSv1_3_client_method())) == NULL) {
        ...
        for (;;) {
            ESP_LOGI(TAG, "wolfSSL_CTX_new file loop");
            vTaskDelay(DelayTicks ? DelayTicks : 1); /* Minimum delay = 1 tick */     
        }
    }
...
/* not peer check */
    if (doPeerCheck == 0) {
        ...
        if ((ret = wolfSSL_CTX_use_certificate_chain_buffer_format(ctx,
            ...
            for (;;) {
                ESP_LOGI(TAG, "wolfSSL_CTX_use_certificate_chain_buffer_format fail loop");
                vTaskDelay(DelayTicks ? DelayTicks : 1); /* Minimum delay = 1 tick */     
                }
        }
...
//        DEFAULT_PORT);
    ...
    if ((ret = connect(sockfd,
        ...
        for (;;) {
            ESP_LOGI(TAG, "socket connect fail loop");
            vTaskDelay(DelayTicks ? DelayTicks : 1); /* Minimum delay = 1 tick */     
        }
    }
    ...
    if (sendGet) {
        ...
        ESP_LOGI(TAG, "message from esp32 tls client\n");
    ...
    if (wolfSSL_read(ssl, buff, sizeof(buff) - 1) == -1) {
        ESP_LOGE(TAG, "ERROR: wolfSSL_read failed to read\n");
        for (;;) {
