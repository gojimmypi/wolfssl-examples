...
* wolfssl unit tests
*
* from https://github.com/wolfSSL/wolfssl/blob/master/IDE/Espressif/ESP-IDF/test/test_wolfssl.c
*/
...
int mp_unitest_mul(const char* strZ, const char* strX, const char* strY, int verbose)
{
    ...
    TEST_ASSERT_EQUAL_STRING_MESSAGE(strZ,
        bufZ,
...
int mp_unitest_mulmod(const char* strZ,
    const char* strX,
    const char* strY,
    const char* strM,
    int verbose)
...
int mp_unitest_exptmod(const char* strZ,
    const char* strX,
    const char* strY,
    const char* strM,
    int verbose)
...
TEST_CASE("wolfssl mp exptmod test", "[wolfssl]")
{
    ...
    TEST_ASSERT_EQUAL(0,
        mp_unitest_exptmod("CE331E6D30A77A57",
            "1234",
            "A",
            "FFFFFFFFFFFFFFFF",
            verbose));
...
TEST_CASE("wolfssl aes sha256 rsa multi-thread test ", "[wolfssl]")
{
    ...
    ESP_LOGI(TAG,
        "Waiting another %llu s. rsa test would take more time to finish.",
